...
2. **设置面板**：Windows 属性窗口式 Tab — `.settings-tab-bar`（`flex-shrink:0` 固定）+ `.settings-pane`（`overflow-y:auto` 独立滚动，非激活 `display:none`）。面板开合状态存 `localStorage('panel-<id>')`。✕ 关闭按钮 `.panel-close-btn`。
   - **字体等宽/衬线开关**：语义化文字（mono：等宽/变宽；serif：衬线/无衬）+ 方形滑块（`.switch .slider` 圆角 4px、宽 58px 适配两字中文、knob 方形 translateX(36px)）；左侧 `.font-switch-label` 标题已移除。**批量替换注意**：模板中其它开关（canvas/行号/标题栏等）也用 `>ON<`/`>OFF<` 文字，全局替换会误伤 — 需按开关类型（`font-mono-toggle`/`font-serif-toggle`/其它）分别处理。
   - **开关对比度与作用域隔离**：字体开关必须限定 `.font-switch-item .switch` 作用域（全局 `.switch` 还原为 46px 胶囊，供画布/行号/标题栏等使用 — 用户要求不能影响其它按钮）。字体开关：**58px 方形**（两字宽度，勿做四字「等宽无衬」）、微软雅黑 **11px**、OFF 文字 `var(--text-color)`、ON 背景 `color-mix(in srgb, var(--theme-accent) 82%, #000)` 压暗配白字（浅紫 accent 也可读）。
   - **tree 连接线（照 input/3.html gemini 参考实现原样移植）**：嵌套 `ul { padding-left:1.4rem }`（缩进由 ul 承担，li 无 padding）；**竖线 `li li::before`**：`left:-0.85rem; top:0; bottom:0`（**用 top/bottom 拉伸而非 height:100%** — 绝对定位伪元素 height:100% 在 Chromium 中不可靠）+ `border-left`，贯穿 li 全高（含子 ul）→ 行间自然连续；**横线 `li li::after`**：`left:-0.85rem; top:1.1rem; width:0.65rem; border-bottom`（行中文字处）；**末节点 `li li:last-child::before`**：`height:1.1rem; bottom:auto` 截断。顶层（根容器）**不画任何引导线**。行容器 row `display:flex; align-items:center`、树链接 `white-space:nowrap`。**紧凑行高**：li `padding-top/bottom: 0.2rem` + `line-height: 1.4`（≈25px/行）。**陷阱：文章树（`.article-tree`）必须同时清除旧 `.tree-node-row { padding: 2px 0 }` 和 `.tree-link { padding: 2px 5px }` 的残留（toc 的 a 无 padding 所以生效、article 的 a 是 .tree-link 有旧 padding 会撑高到 33px — 新段统一 `a { padding: 0 4px }`，active 胶囊除外）；**横线与末节点截断位置必须用 `calc(0.2rem + 0.7em)`**（padding + 行高一半，自动跟随，改行距不错位 — 用固定 `top:1.1rem` 会在改行高后错位）。**必须清除旧 `.toc-tree .toc-tree { padding-left:0 }` 与 `.article-tree ul { border-left:dashed }` 规则**（特异性更高会破坏缩进/残留虚线）。线色 `var(--theme-border)`；折叠三角 `display:none`；选中胶囊（`var(--theme-link)` 20% tint + radius 6px）；标题内 code 小胶囊（surface2 底 + 边框 + link 色 + radius 6px）。
   - **标题 H 标记**：`#article-container hN::after { content:'H1'... }` hover 时 opacity:1，left:-52px 悬出版面左侧。**用 ::after**（lightmind 主题的 `h3::before` 装饰条占用 ::before，避免冲突）。
